...
public class TestHnswGraph extends LuceneTestCase {
  ...
  public void testHnswGraphBuilderInitializationFromGraph_withOffsetZero() throws IOException {
    ...
    RandomVectorValues finalVectorValues =
        new RandomVectorValues(
            totalSize, dim, vectorEncoding, random(), initializerVectors.values, docIdOffset);
    ...
    HnswGraphBuilder<?> finalBuilder =
        HnswGraphBuilder.create(
            finalVectorValues, vectorEncoding, similarityFunction, 10, 30, seed);
  ...
  private Map<Integer, Integer> createOffsetOrdinalMap(
      int docIdSize, VectorValues totalVectorValues, int docIdOffset) throws IOException {
